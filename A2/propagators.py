...
def prop_FC(csp, newVar=None):
    ...
    for constraint in constraints:
        # Only add constraint with one unssigned variable
        if len(constraint.get_unasgn_vars()) == 1:
            var = constraint.get_unasgn_vars()[0]

            # Prune value for the ussigned variable
            for d in var.cur_domain():
                var.assign(d)
                lst = []
                for v in constraint.get_scope():
                    lst.append(v.get_assigned_value())
                    
                if constraint.check(lst) == False:
                    var.prune_value(d)
                    pruned.append((var, d))
...
def ord_mrv(csp):
    ...
    if not var:
        ...
        res = float('inf')
        for v in var:
            if v.cur_domain_size() < res:
                res = v.cur_domain_size()
                ret = v
        return ret
